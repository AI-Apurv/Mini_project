...
export class UserController {
  ...
  async signup(@Body(new ValidationPipe({ transform: true })) signupDto: SignupDto, @Res() response: Response) {
    ...
    const resp = this.httpResponse.sendResponse(response, USERRESPONSE.SIGNUP_SUCCESS,)
    return resp;
  }
  ...
  @ApiOperation({ summary: 'User Login' })
  @Post('login')
  async login(@Body(new ValidationPipe()) loginDto: LoginDto, @Res() response: Response) {
    const user = await this.userService.validateUser(loginDto.email, loginDto.password);

    if (!user) {
      return this.httpResponse.sendResponse(response, USERRESPONSE.NOT_FOUND)
    }

    const payload = { sub: user.id, email: user.email, role: user.role, userStatus:user.userStatus};
    const accessToken = this.jwtService.sign(payload);
    await client.connect();
    await client.set(user.id.toString(), 'true')
    const resp = this.httpResponse.sendResponse(response, USERRESPONSE.LOGIN_SUCCESS, accessToken)
    return resp;
  }
